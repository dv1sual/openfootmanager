                ...
                {isOwnClub ? (
                                ...
                                <div className="flex flex-col gap-2">
                                    {group.attrs.map((attr) => (
                                        <div key={attr.name} className="flex items-center gap-2">
                                            ...
                                            />
                                            <span
                                                className={`font-heading font-bold text-xs w-6 text-right tabular-nums shrink-0 ${getAttributeColorClass(attr.value)}`}
                                            >
                                                {attr.value}
                                            </span>
                                        </div>
                                    ...
                                    <div className="pt-1 border-t border-gray-100 dark:border-navy-600 flex items-center gap-2">
                                        <span className="text-xs text-gray-500 dark:text-gray-400 w-20 shrink-0 font-semibold">
                                            {averageLabel}
                                        </span>
                                        <span className="flex-1" />
                                        <span className="font-heading font-bold text-xs w-6 text-right tabular-nums shrink-0 text-gray-700 dark:text-gray-200">
                                            {group.average}
                                        </span>
                                    </div>
                                </div>
                    ...
                    <div className="text-center py-8">
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-4 text-left">
                            {attrGroups.map((group) => (
                                <div key={group.label}>
                                    <h4 className="font-heading font-bold text-xs uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2 pb-2 border-b border-gray-100 dark:border-navy-600">
                                        {group.label}
                                    </h4>
                                    {group.attrs.map((attr) => (
                                        <div key={attr.name} className="flex items-center gap-2 mb-2">
                                            <span className="text-xs text-gray-400 dark:text-gray-500 w-20 shrink-0">
                                                {attr.name}
                                            </span>
                                            <div className="flex-1 min-w-0 h-1.5 bg-gray-200 dark:bg-navy-600 rounded-full overflow-hidden">
                                                <div
